...
!["Rplot for crimes resulting from assault"](c:/Users/Nisha/Desktop/Data Cleaning/Rplot_assault.png)
...
!["Rplot for crimes resulting from drug abuse"](c:/Users/Nisha/Desktop/Data Cleaning/Rplot_drug abuse.jpeg)
...
!["Rplot for crimes resulting from curfew"](c:/Users/Nisha/Desktop/Data Cleaning/Rplot_curfew crimes.jpeg)
...
### Mapping using ggplot and qplot ####
### Trail and error sample ####

```library("ggplot2")```

```ggplot(d, aes(d$NE_Unemployment_Rate))```

```qplot(d$NE_Unemployment_Rate, d$Crime_Aggravated_Assault, data=d, alpha=I(.5), col='red',
      main="Relation between the unemployment rate in NE and assault crimes ", xlab="Unemployment rate", 
      ylab="Assault crimes")```
!["plot for crimes resulting from assault"](c:/Users/Nisha/Desktop/Data Cleaning/ggplot 1.jpeg)

```qplot(d$NE_Unemployment_Rate, d$Crime_Aggravated_Assault+d$Crime_Drug_Abuse_Violations+d$Crime_Curfew, data=d, geom=c("boxplot","jitter"), 
      fill=d$Crime_Aggravated_Assault+d$Crime_Drug_Abuse_Violations+d$Crime_Curfew, col='blue', main="Unemp rate vs correlated crimes",
      xlab="Unemployment rate in NE", ylab="Crimes correlated to unemp rate")```
      

```ggplot(d, aes(x = d$NE_Unemployment_Rate, fill = d$NE_Unemployment_Rate))+
  geom_line(aes(y = d$Crime_Aggravated_Assault, fill = d$Crime_Aggravated_Assault))+geom_bar()```


```ggplot(d, aes(x = d$NE_Unemployment_Rate))+
  geom_line(aes(y = d$Crime_Aggravated_Assault))+
  geom_line(aes(y = d$Crime_Curfew))+
  geom_line(aes(y = d$Crime_Drug_Abuse_Violations))+geom_bar()```
  

```ggplot(d, aes(x = d$NE_Unemployment_Rate))+
  geom_boxplot(aes(y = d$Crime_Aggravated_Assault))+
  geom_boxplot(aes(y = d$Crime_Curfew))+
  geom_boxplot(aes(y = d$Crime_Drug_Abuse_Violations))+geom_bar()```
