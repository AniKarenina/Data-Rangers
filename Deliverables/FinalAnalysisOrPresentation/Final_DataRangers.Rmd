...
lawCrime <- fread('https://raw.githubusercontent.com/Shivani-Parihar/Data-Rangers/master/Deliverables/DataCleaningDeliverable/clean_data/lawStaff_vs_Crime.csv')
...
ggscatter(lawCrime, x = "pop_covered", y = "crime_per_1000", 
          ...
          xlab = "Total Population Covered", ylab = "Crime per 1000")
```
```{r}
ggscatter(lawCrime, x = "total_so", y = "crime_per_1000", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "Total Sworn Officers", ylab = "Crime per 1000")
```



**Looking at how the officer per 1000 effects crime rate per 1000** 
```{r echo=FALSE}
# Remove entries with Inf for plotting
lawCrimeNoInf <- lawCrime
# lapply(lawCrimeNoInf, function(x) x[is.finite(x)])
lawCrimeNoInf <- lawCrimeNoInf[Reduce(`&`, lapply(lawCrimeNoInf, function(x) !is.infinite(x))),]
```

```{r}
pCor <- cor(lawCrime$so_per_1000, lawCrime$crime_per_1000, method="pearson")

# Regressing officers per 1000 and crime rate per 1000
line <- lm(so_per_1000 ~ crime_per_1000, data = lawCrimeNoInf)
plot(crime_per_1000 ~ so_per_1000, data = lawCrimeNoInf,
pch = 16, cex = 0.3, col = "blue", 
main = "SO/1000 & Crime/1000", 
xlab = "SO per 1000", 
ylab = "Crime per 1000") +
abline(line)
```
